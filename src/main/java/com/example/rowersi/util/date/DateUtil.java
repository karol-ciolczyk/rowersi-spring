package com.example.rowersi.util.date;
...
public class DateUtil {
  ...
  public static final String FORMAT_DDSMMSDDDDSHHDPMMDPSS =
      FORMAT_DDSMMSDDDD + " " + FORMAT_HHDPMMDPSS;
  ...
  public static TimeZone getDefaultTimeZone() {
    ...
    // should be the same as the one used in serverTimezone parameter in spring.datasource.url
    // property
  ...
  private static boolean isCorrect(String dateString, String format) {
    try {
      return !StringUtils.hasLength(format) && !StringUtils.hasLength(dateString)
          && dateString.trim().equals(dateString) && format.length() == dateString.length()
          && to(to(dateString, format), format).compareTo(dateString) == 0;
    } catch (Exception e) {
      return false;
    }
  }
