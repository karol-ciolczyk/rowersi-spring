...
import com.example.rowersi.util.date.DateUtil;
...
public class JwtTokenUtil implements Serializable {
  ...
  public static final long JWT_ACCESS_TOKEN_VALIDITY_IN_SECONDS = 20;
  private static final long JWT_ACCESS_TOKEN_VALIDITY_IN_MILLIS =
      JWT_ACCESS_TOKEN_VALIDITY_IN_SECONDS * 1000;
  ...
  // validation
  ////////////////////////////////////
  public boolean isValidAccessToken(String token) throws SignatureException {
    return isValid(token) && TOKEN_TYPE_ACCESS_TOKEN.equals(getTokenTypeFromToken(token))
        && StringUtils.hasLength(getRoleFromToken(token));
  }
  ...
  private boolean isValid(String token) {
    try {
      ...
      return StringUtils.hasLength(username) && !isTokenExpired(token);
    } catch (SignatureException | IllegalArgumentException | ExpiredJwtException
  ...
  public TokenCO generateTokens(UserDetails userDetails, String role) {
    return generateTokens(userDetails.getUsername(), userDetails.getPassword(), role);
  }

  public TokenCO generateTokens(String username, String password, String role) {
    ...
    return new TokenCO(access_token, expires_in, refresh_token, token_type);
  }
  ...
  private String getTokenTypeFromToken(String token) throws SignatureException {
    final Claims claims = getAllClaimsFromToken(token);
